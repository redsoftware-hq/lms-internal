"""Friendlier, safer password-reset endpoint.

Overrides the stock ``reset_password`` (wired via ``override_whitelisted_methods``
in hooks.py) to:
  - never leak whether an account exists (same message regardless),
  - swallow outgoing-email / unexpected errors so the user isn't shown a stack
    trace, logging them instead,
  - show a clear "check your inbox" confirmation.
"""
