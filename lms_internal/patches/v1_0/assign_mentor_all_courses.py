"""Add the default mentor (Samarth) as instructor on all existing courses.

Appends to the ``instructors`` child table on each ``LMS Course`` where
samarth@qualityasia.in is not already listed.  Future courses are handled by a
``before_insert`` hook (see hooks.py / overrides/mentor.py).

No-op if the mentor user doesn't exist yet. Idempotent — safe to re-run.
"""
