...
Usage:
    python lms_internal/setup/tools/extract_quiz_from_forms.py \\
...
Outputs (relative to the repo root — run from the repo root):
    lms_internal/setup/data/questions_iso_auditor.json  (regenerated)
    lms_internal/setup/data/quizzes_iso_auditor.json    (regenerated)
    lms_internal/setup/data/courses_iso_auditor.json    (feedback chapters added)
    lms_internal/setup/data/quiz_extraction_report.md   (confidence review)
...
Generated by `lms_internal/setup/tools/extract_quiz_from_forms.py`.
...
if __name__ == "__main__":
	...
	if not forms_root.is_dir():
		...
		sys.exit(1)
	# repo root = two levels up from this script (tools/ → setup/ → lms_internal/ → repo root)
